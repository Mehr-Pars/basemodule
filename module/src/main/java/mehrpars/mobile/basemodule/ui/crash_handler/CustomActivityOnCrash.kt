package mehrpars.mobile.debugtools.ui.activity.error_activity
...
/**
 * این کلاس برای مدیریت داخلی کرش های برنامه است
 * در زمان بروز خطاهایی که باعث کرش کردن برنامه میشود
 * به جای Crash Handler پیش فرض اندروید، این
 * کلاس وظیفه هندل کردن خطا را دارد
 */
@SuppressLint("NewApi")
object CustomActivityOnCrash {
    ...
    private val EXTRA_RESTART_ACTIVITY_CLASS by lazy { application?.packageName + ".EXTRA_RESTART_ACTIVITY_CLASS" }

    //        BuildConfig.LIBRARY_PACKAGE_NAME + ".EXTRA_RESTART_ACTIVITY_CLASS"
    private val EXTRA_SHOW_ERROR_DETAILS by lazy { application?.packageName + ".EXTRA_SHOW_ERROR_DETAILS" }

    //        BuildConfig.LIBRARY_PACKAGE_NAME + ".EXTRA_SHOW_ERROR_DETAILS"
    private val EXTRA_STACK_TRACE by lazy { application?.packageName + ".EXTRA_STACK_TRACE" }

    //    BuildConfig.LIBRARY_PACKAGE_NAME + ".EXTRA_STACK_TRACE"
    private val EXTRA_EVENT_LISTENER by lazy { application?.packageName + ".EXTRA_EVENT_LISTENER" }
//        BuildConfig.LIBRARY_PACKAGE_NAME + ".EXTRA_EVENT_LISTENER"

    //General constants
    private val INTENT_ACTION_ERROR_ACTIVITY by lazy { application?.packageName + ".ERROR" }

    //        BuildConfig.LIBRARY_PACKAGE_NAME + ".ERROR"
    private val INTENT_ACTION_RESTART_ACTIVITY by lazy { application?.packageName + ".RESTART" }

    //        BuildConfig.LIBRARY_PACKAGE_NAME + ".RESTART"
    private val CAOC_HANDLER_PACKAGE_NAME by lazy { application?.packageName ?: "" }
    ...
    private const val TIMESTAMP_DIFFERENCE_TO_AVOID_RESTART_LOOPS_IN_MILLIS = 5000
                ...
                if (oldHandler != null &&
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(Thread.UncaughtExceptionHandler { thread, throwable ->
                        ...
                        if (hasCrashedInTheLastSeconds(application)) {
                            ...
                            } else if (isLaunchErrorActivityWhenInBackground || !isInBackground) {
    ...
    fun getAllErrorDetailsFromIntent(context: Context, intent: Intent): String {
        ...
        errorDetails += "Build expireDate: $buildDateAsString \n"
        errorDetails += "Current expireDate: " + dateFormat.format(currentDate) + " \n"
    ...
    fun getEventListenerFromIntent(intent: Intent): EventListener? {
        val serializedClass =
            intent.getSerializableExtra(EXTRA_EVENT_LISTENER)
    ...
    /**
     * INTERNAL method that returns the build expireDate of the current APK as a string, or null if unable to determine it.
     *
     * @param context    A valid context. Must not be null.
     * @param dateFormat DateFormat to use to convert from Date to String
     * @return The formatted expireDate, or "Unknown" if unable to determine it.
     */
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="ir.nwms.samt.RESTART"></action>,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="ir.nwms.samt.RESTART"></action>,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="ir.nwms.samt.ERROR"></action>,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="ir.nwms.samt.ERROR"></action>,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
